...
<div class="d-none show-table" id="searchedUsersResultsRow">
	<table id="searchedUsersResultsTable" class="table table-bordered" style="width:96%;">
		...
		$(document).on('click', '.get-single-ticket', function(e)
        {
            ...
            $.ajax({
				...
				error: function(xhr) {
					...
					Swal.fire('Another Ticket is being processed.Please try again after some seconds.');
		...
		$(document).on('click', '.update-user-details', function(e)
        {
            ...
            $.ajax({
                ...
                success: function(response) {
					...
					if(response.staff_fname !== null)
					{
						$('#createdAtDetails').removeClass('d-none').addClass('d-flex');

						$('#staffName').val(response.staff_fname+' '+response.staff_lname);

						$('#staffDateCreated').val(response.staff_date);
						
					} else {
						$('#createdAtDetails').removeClass('d-flex').addClass('d-none');
					}
                }
            });
        });
