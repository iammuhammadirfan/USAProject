...
<!-- <div class="card p-3 shadow"> -->
    <h2 class="text-center p-3">Update Staff Account Password</h2>
    <div style="display: flex;flex-direction: column;gap: 5px;align-items: center;">
        ...
        <?php use Illuminate\Support\Facades\DB;
            ...
            $staff_details=DB::table('users')
            ->select('first_name','last_name','case_number','id')
            ->where('id',$id)
            ->first();
        ?>
        <input type="hidden" class="user-id" readonly value="{{ $staff_details->id }}">
        
        <div class="form-group">
            <label>First name</label>
            <input type="text" class="form-control text-dark" readonly value="{{ $staff_details->first_name }}">
        </div>
        <div class="form-group">
            <label>Last name</label>
            <input type="text" class="form-control text-dark" readonly value="{{ $staff_details->last_name }}">
        </div>
        <div class="form-group">
            <label>Current Password</label>
            <input type="text" class="form-control text-dark" readonly value="{{ $staff_details->case_number }}">
        </div>
        <div class="form-group pwd-holder">
            <label>New Password</label>
            <input type="text" class="form-control text-dark" id="newPwd" placeholder="New Password">
            <i class="fas fa-eye-slash" id="toggleNewPwd"></i>
        </div>
        <div class="form-group pwd-holder">
            <label>Confirm Password</label>
            <input type="text" class="form-control text-dark" id="confirmPwd" placeholder="Confirm Password">
            <i class="fas fa-eye-slash" id="toggleConfirmPwd"></i>
        </div>
        <span id="correctPwd" style="font-size: 20px;" class="text-danger"></span>
        <ul style="list-style-type: none;" class="alert alert-danger d-none error-list"></ul>
        <button type="submit" class="btn btn-warning btn-lg mb-2 p-2 update-staff-pwd">Save</button>
    </div>
<!-- </div> -->
@endsection

@section('update_pwd_script')
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('toggleNewPwd').addEventListener('click', function() {
            var icon = document.getElementById("toggleNewPwd");
            var passwordField = document.getElementById('newPwd');
        ...
        document.getElementById('toggleConfirmPwd').addEventListener('click', function() {
            var icon = document.getElementById("toggleConfirmPwd");
            var passwordField = document.getElementById('confirmPwd');
            if (passwordField.type === 'password') 
            {
                passwordField.type = 'text';
                icon.classList.remove("fa-eye-slash");
        ...
        $(document).on('keyup','#newPwd',function(e)
        {
            ...
            console.log(typedPwd);

            var errors = [];

            if(typedPwd.length < 6)
            {
                var msg = 'Password must have a minimum of 6 characters';

                errors.push(msg);
            }

            if (!hasNumber(typedPwd)) {
                var msg = 'Password must have at least 1 number';

                errors.push(msg);
            }

            if (!hasLetter(typedPwd)) {
                var msg = 'Password must have at least 1 letter.';

                errors.push(msg);
            }

            if (!hasSpecialChars(typedPwd)) {
                var msg = 'Password must have at least 1 special character (i.e.  ! @ # $ %)';
        ...
        {
            var typedNewPwd = $('#newPwd').val();

            var typedPwd = $(this).val();

            if(typedPwd !== typedNewPwd)
            {
                var msg = 'Confirm Password should be similar to the new password.';

                $('#correctPwd').html(msg);
            } else {
                var msg = 'Confirm Password matches the new password.';
        ...
        $(document).on('click','.update-staff-pwd',function(e){
            ...
            {
                Swal.fire('Password Mismatch for new and confirm password inputs');
            }
            ...
            $.ajax({
                url: '/admin/update-admin-staff-password',
                type: 'POST',
                data: postData,
                dataType: 'json',
