...
class AdminController extends Controller
{
    ...
    public function add_volunteer_users(Request $request)
    {
        $data = $request->all();
        $date_created = Carbon::now()->toDateTimeString();
        ...
        $rules = [
            ...
            'case_numbers.*' => 'required|string'
        ];

        $custommessages = [
            'first_name.required' => 'First Name is required',
            'last_name.required' => 'Last Name is required',
            'case_numbers.required' => 'At least one case number is required',
        ...
        if ($validator->fails()) {
            ...
            if ($user_count > 0) {
                ...
                if ($data['user_id'] == null) {
                    ...
                    foreach ($data['case_numbers'] as $caseNumber) {
                        DB::table('users') // Replace with your actual table name
                    ...
                    $msg = 'User updated successfully.';
                    // 🔥 Link each provided case number with this volunteer ID
                    return response()->json([
                        'status' => 200,
                        'id' => $userId,
                        'message' => $msg
