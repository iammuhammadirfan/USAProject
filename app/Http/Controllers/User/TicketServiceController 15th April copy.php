...
public function manage_tickets_generation(Request $request)
{
    if ($request->wantsJson()) {
        $request->merge(['_token' => $request->input('_token')]);
    }
    ...
    try {
        ...
        if ($redisTicketNumber - $lastNumber > 1) {
            \Log::error("Ticket number gap detected: Last=$lastNumber, New=$redisTicketNumber");
            $redis->set($ticketNumberKey, $redisTicketNumber);
        }
        ...
        if ($submissionType == 1) 
        {
            ...
            $projectedReturnTime=array();
                    ...
                    // $newTicket=$redis->incrby("tickets:count:$todaysDate", $ticketNumber + count($userIdsArray));

                    // \Log::info('Next Ticket is '.$newTicket);
            ...
            // $redis->set($ticketNumberKey, $reversedTicket);
            ...
            if($todaysDate == $latestSavedDate)
            {
                ...
                if(count($getDeactivatedTicketNumbers)>0)
                {
                    ...
                    {
                        $startingValue = $redisTicketNumber;

                        for ($i = 0; $i < $userIdsCount; $i++) {
                            $newTicketsArray[] = $startingValue + $i;
                        }

                        $redis->set("tickets:counter:$todaysDate", $redisTicketNumber + count($tickets) - 1);

                        \Log::info('Next Ticket is update is '.$redisTicketNumber + count($tickets) - 1);
                ...
                // {
                //     $newTicketsArray[] = array_values(array_slice($msnTicketsArray, 0, count($msnTicketsArray)));;
                // }
                else 
                {
                    \Log::info('Next Ticket to be generated is '.$redisTicketNumber); 
                    ...
                    $redis->set("tickets:counter:$todaysDate", $redisTicketNumber + count($tickets) - 1);
                        ...
                        \Log::info('Next Ticket in line is '.$redisTicketNumber + count($tickets) - 1);
            ...
            } else if (count($msnTicketsArray)>0)
            {
                ...
                $reversedTicket = $redisTicketNumber;
            ...
            // if (count($msnTicketsArray) == 0)
            // {
            //     $redis->incrby("tickets:count:$todaysDate", $ticketNumber + 1);
            // }

                                    // $newTicket=$redis->incrby("tickets:count:$todaysDate", $ticketNumber + 1);
        ...
        if($request->submission_type !== null && $request->submission_type == 1)
        {
            ...
            $response_msg = 'Single ticket: Time' . $seconds . ' seconds and microseconds ' . $microseconds.', ticket:'.$ticketNumber.', Last Ticket:'.$lastNumber;
