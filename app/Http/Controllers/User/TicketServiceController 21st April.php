...
public function manage_tickets_generation(Request $request)
{
    ...
    $circuitBreaker = new CircuitBreaker($redis, 2, 10); 

    DB::beginTransaction();

    try {
        // Redis lock to prevent concurrent processing
        $lockKey = "ticket_gen_lock:" . ($request->submission_type == 1 ? 'multi' : $request->userId);
        ...
        if (!$redis->exists($ticketNumberKey)) {
            $maxNumber = DB::table('generated_tickets')
                ...
                ->lockForUpdate()
                ->max('ticket_number') ?? 0;
            $redis->set($ticketNumberKey, $maxNumber, 'NX');
        }

        // Atomic ticket number assignment
        $ticketAssignment = function() use ($redis, $ticketNumberKey, $todaysDate) {
            return DB::transaction(function () use ($redis, $ticketNumberKey, $todaysDate) {
                // Get next number
                $nextNumber = $redis->incr($ticketNumberKey);
                
                // Verify uniqueness with database lock
                ...
                if ($exists) {
                    // If duplicate, find next available gap
                    $nextAvailable = DB::table('generated_tickets as t1')
                        ->select(DB::raw('t1.ticket_number + 1 as gap'))
                        ->leftJoin('generated_tickets as t2', function($join) use ($todaysDate) {
                            $join->on(DB::raw('t1.ticket_number + 1'), '=', 't2.ticket_number')
                                ->whereDate('t2.created_at', $todaysDate);
                        })
                        ->whereDate('t1.created_at', $todaysDate)
                        ->whereNull('t2.ticket_number')
                        ->orderBy('t1.ticket_number')
                        ->value('gap');
        ...
        // 1. Get the highest existing ticket number with LOCK
        $maxTicket = DB::table('generated_tickets')
            ->whereDate('created_at', $todaysDate)
            ->lockForUpdate()
            ->max('ticket_number') ?? 0;

        // 2. Find gaps in existing tickets
        $nextAvailable = DB::table('generated_tickets as t1')
            ->select(DB::raw('t1.ticket_number + 1 as gap'))
            ->leftJoin('generated_tickets as t2', function($join) use ($todaysDate) {
                $join->on(DB::raw('t1.ticket_number + 1'), '=', 't2.ticket_number')
        ...
        $redisTicketNumber = $nextAvailable ?: ($maxTicket + 1);

                    // $redisTicketNumber = $circuitBreaker->attempt($ticketAssignment);
                    
                    // // Fallback to database if Redis fails or circuit is open
                    // if (!$redisTicketNumber || $circuitBreaker->getState() === 'open') {
                    //     DB::beginTransaction();
                    //     try {
                    //         \Log::warning("Using database fallback for ticket generation");
                    ...
                    //         // Your existing database fallback logic...
                    //         $nextAvailable = DB::table('generated_tickets as t1')
                    //             ->select(DB::raw('t1.ticket_number + 1 as gap'))
                    //             ->leftJoin('generated_tickets as t2', function($join) use ($todaysDate) {
                    //                 $join->on(DB::raw('t1.ticket_number + 1'), '=', 't2.ticket_number')
                    ...
                    //         $redisTicketNumber = $nextAvailable ?: ($maxTicket + 1);
                    //         // if (!$redisTicketNumber) {
                    //         //     $redisTicketNumber = DB::table('generated_tickets')
                    //         //         ->whereDate('created_at', $todaysDate)
                    //         //         ->lockForUpdate()
                    //         //         ->max('ticket_number');
                    //         //     $redisTicketNumber = $redisTicketNumber ? $redisTicketNumber + 1 : 1;
                    //         // }
                            
                    //         DB::commit();
                            
                    //         // If circuit is half-open and DB fallback worked, reset circuit
        ...
        // After successful database fallback, try to update Redis
        if ($circuitBreaker->getState() !== 'open') {
            try {
                $redis->set($ticketNumberKey, $redisTicketNumber);
            } catch (\Exception $e) {
                \Log::warning("Failed to update Redis after database fallback: " . $e->getMessage());
        ...
        do {
            try {
                DB::transaction(function () use ($todaysDate, &$redisTicketNumber, $redis, $ticketNumberKey) {
                    // Check for duplicate
                    $exists = DB::table('generated_tickets')
                        ->whereDate('created_at', $todaysDate)
                        ->where('ticket_number', $redisTicketNumber)
                        ->lockForUpdate()
                        ->exists();

                    if ($exists) {
                        // If duplicate, get next available number
                        $nextAvailable = DB::table('generated_tickets as t1')
                            ->select(DB::raw('t1.ticket_number + 1 as gap'))
                            ->leftJoin('generated_tickets as t2', function($join) use ($todaysDate) {
                                $join->on(DB::raw('t1.ticket_number + 1'), '=', 't2.ticket_number')
                        ...
                        // Update Redis to prevent future duplicates
                        $redis->set($ticketNumberKey, $nextAvailable);
                        $redisTicketNumber = $nextAvailable;

                        // $ticketNumber = $nextAvailable;
                        
                        throw new \Exception("Duplicate resolved - using $nextAvailable");
                    }
                });
                ...
                if ($attempt >= $maxRetries) {
                    throw new \Exception("Failed to generate unique ticket after $maxRetries attempts");
                }
        ...
        //     $newTicketNumber = DB::table('generated_tickets')
        //     ->whereDate('created_at', $todaysDate)
        //     ->max('ticket_number') + 1;

        //     // $redisTicketNumber=$newTicketNumber;
        //     $ticketNumber=$newTicketNumber;

        //     $redis->set($ticketNumberKey, $ticketNumber);

        //     \Log::error("Critical error: Duplicate ticket number '.$redisTicketNumber.' ,
        //     New Ticket will for the user is ".$ticketNumber);
        ...
        { 
            ...
            if($todaysDate == $latestSavedDate)
            {
                ...
                if(count($getDeactivatedTicketNumbers)>0)
                {
                    ...
                    {
                        $currentDbTicket = $redisTicketNumber - 1;

                        $new_ticket_counter = $currentDbTicket + count($tickets);

                        $redis->set("tickets:counter:$todaysDate", $new_ticket_counter);

                        $next_ticket=$redis->get('tickets:counter:$todaysDate');

                        \Log::info('Next Ticket in queue is '.$next_ticket.' after users '.json_encode($userIdsArray).' when user tickets are greater than deleted tickets');

                        for ($i = 0; $i < $userIdsCount; $i++) {
                            $newTicketsArray[] = $currentDbTicket + $i;
                        }
                    }
                    ...
                    if(count($tickets) < count($getDeactivatedTicketNumbers))
                    {
                        ...
                        $redis->set("tickets:counter:$todaysDate", $currentDbTicket);

                        $next_ticket=$redis->get('tickets:counter:$todaysDate');

                        \Log::info('Next Ticket in queue is '.$next_ticket.' after users '.json_encode($userIdsArray).' when user tickets are less than deleted tickets');
                    ...
                    if(count($tickets) == count($getDeactivatedTicketNumbers))
                    {
                        // $new_ticket_counter = $redisTicketNumber;
                        $currentDbTicket = $redisTicketNumber;

                        $redis->set("tickets:counter:$todaysDate", $currentDbTicket - 1);

                        $next_ticket=$redis->get('tickets:counter:$todaysDate');

                        \Log::info('Next Ticket in queue is '.$next_ticket.' after users '.json_encode($userIdsArray).' when user tickets and deleted tickets are equal');
                ...
                else 
                {
                    ...
                    $new_ticket_counter = $currentDbTicket + count($tickets) - 1;

                    $redis->set("tickets:counter:$todaysDate", $new_ticket_counter);

                    $next_ticket=$redis->get('tickets:counter:$todaysDate');

                    \Log::info('Next Ticket in queue is '.$next_ticket.' after users '.json_encode($userIdsArray).' when there are no deleted tickets');
        ...
        {
            $currentTicket = $redis->get($ticketNumberKey);
            ...
            if($ticketCount > 0)
            {
                $redis->set("tickets:counter:$todaysDate", $redisTicketNumber - 1);
            ...
            if ($deactivatedTicket) {
                ...
                $next_ticket = $redis->set("tickets:counter:$todaysDate", $redisTicketNumber - 1);

                $current_ticket=$redis->get('tickets:counter:$todaysDate');

                \Log::info('Next Ticket in queue is '.$next_ticket.' after user '.$request->userId.' replaced by 
                deleted ticket '.$ticketNumber.'
                current ticket is '.$redisTicketNumber);
            } 
            else if (count($msnTicketsArray)>0)
            {
                $ticketNumber=$msnTicketsArray[0];

                $finalTicketInArray=array_slice($msnTicketsArray, - 1)[0];
                
                $redis->set("tickets:counter:$todaysDate", $finalTicketInArray);

                $current_ticket=$redis->get('tickets:counter:$todaysDate');

                \Log::info('Next Ticket in queue is '.$finalTicketInArray.' after user '.$request->userId.' replaced by 
                skipped tickets '.json_encode($msnTicketsArray).'
                replaced ticket is '.$ticketNumber.'
                current ticket is '.$finalTicketInArray);
            } 
            else if (count($msnTicketsArray) == 0)
            {
                ...
                if($redisTicketNumber == 0)
                {
                    ...
                    $ticketNumber=$redisTicketNumber;
                }

                // $next_ticket = $ticketNumber;

                $redis->set("tickets:counter:$todaysDate", $ticketNumber);
    
                $current_ticket=$redis->get('tickets:counter:$todaysDate');

                \Log::info('Next Ticket in queue is '.$ticketNumber.' after user '.$request->userId);
            } 
        ...
        if ($submissionType == 1) 
        {
            DB::transaction(function () use ($tickets, $newTicketsArray, $request) {
                $isBot = $request->session()->pull('is_bot', false); // Get and clear in one operation
                
                foreach ($tickets as $key => $ticketId) {
                    $update = ['ticket_number' => $newTicketsArray[$key]];
            ...
            DB::table('generated_tickets')
                ->whereIn('user_id', $extraSelectedTickets)
        ...
        {
            $response_time = 'Multiple tickets: ' . $seconds . ' seconds and microseconds ' . $microseconds;
            ...
            \Log::info("Ticket assigned", [
                'number' => $redisTicketNumber,
                'user' => $request->userId,
                'time' => now(),
                'source' => ($circuitBreaker->getState() === 'closed') ? 'redis' : 'db_fallback'
            ]);
        ...
        \Log::error("Ticket generation failed: " . $e->getMessage().'. Ticket wasnt Generated');
        return response()->json([
            'status' => 550,
            'message' => 'Another ticket process is already in progress. Please wait.'
        ], 550);
...
public function create_one_ticket_details(Request $request)
{
    ...
    //     $ticketNumber = DB::table('generated_tickets')
    //     ->selectRaw('COALESCE(MAX(ticket_number), 0) + 1 AS next_number')
    //     ->whereDate('created_at', $todaysDate)
    //     ->value('next_number');

    //     // Handle deactivated tickets (your existing logic)
    //     $deactivatedTicket = DB::table('generated_tickets')
    //     ->whereDate('created_at', $todaysDate)
    //     ->where(['is_active' => 0, 'status' => 1])
    //     ->where('is_cancelled', '!=', 0)
