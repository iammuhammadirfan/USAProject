...
public function manage_tickets_generation(Request $request)
{
    ...
    try {
        ...
        // Try Redis first
        $retries = 3;
        $redisTicketNumber = null;

        while ($retries-- > 0) {
            try {
                $redisTicketNumber = $redis->incr($ticketNumberKey);

                \Log::error("first try = $redisTicketNumber");

                break;
            } catch (\Exception $e) {
                \Log::warning("Redis increment failed (retries left: {$retries}): ".$e->getMessage());
        ...
        if (!$redisTicketNumber) {
            ...
            try {
                ...
                $redisTicketNumber = DB::table('generated_tickets as t1')
                    ->select(DB::raw('t1.ticket_number + 1 as next_available'))
                ...
                if (!$redisTicketNumber) {
                    ...
                    \Log::error("second try = $redisTicketNumber");
                }
                
                
                // Update Redis with the new number
                // try {
                //     if ($submissionType == 1) 
                //     {
                ...
                // } catch (\Exception $e) {
                //     \Log::error("Failed to update Redis counter: ".$e->getMessage());
                // }
        ...
        $duplicateCheck = DB::table('generated_tickets')
            ->whereDate('created_at', $todaysDate)
        ...
        if ($duplicateCheck) {
            ...
            \Log::error("Critical error: Duplicate ticket number {$redisTicketNumber} ,Next Ticket will be $ticketNumber");
        }

        // // Immediately verify uniqueness
        ...
        // if ($exists) 
        // {
        //     // If duplicate, get next available number from DB
        //     $ticketNumber = DB::table('generated_tickets')
        //         ->whereDate('created_at', $todaysDate)
        //         ->max('ticket_number') + 1;

        //     // Update Redis to this new number
        //     $redis->set($ticketNumberKey, $ticketNumber);
        // }
        ...
        //     // Update Redis to maintain consistency
           
        // }

        // if ($duplicateCheck) {

        //     throw new \Exception("Critical error: Duplicate ticket number {$redisTicketNumber} detected");

        //     \Log::error("duplicate numer = $redisTicketNumber");
        //     // throw new \Exception("Critical error: Duplicate ticket number {$redisTicketNumber} detected");
        //     // return response()->json([
        //     //     'status'=>530,
        //     //     'message'=>'Your request has been added to queue.Please try again after a few seconds.'
        //     // ]);
        //     // \Log::error("Ticket number gap detected: Last=$lastNumber, New=$redisTicketNumber");
        // }
        ...
        if ($submissionType == 1) 
        {
            ...
            if($todaysDate == $latestSavedDate)
            {
                ...
                if(count($getDeactivatedTicketNumbers)>0)
                {
                    ...
                    if(count($tickets) > count($getDeactivatedTicketNumbers))
                    {
                        ...
                        \Log::info("Next Ticket will be $next_ticket");
                    ...
                    if(count($tickets) < count($getDeactivatedTicketNumbers))
                    {
                        ...
                        \Log::info("Next Ticket will remain as $next_ticket");
                ...
                //     \Log::info("Current Ticket is $currentDbTicket,Next Ticket after multiple tickets duplicate will be $new_ticket_counter");
                ...
                else 
                {
                    ...
                    \Log::info("Current Ticket is $currentDbTicket,Next Ticket after multiple tickets will be $new_ticket_counter");
            ...
            // dd($tickets,$todaysDate,$latestSavedDate,$getDeactivatedTicketNumbers,$ticketNumber,$userIdsCount,$newTicketsArray);die();
            ...
            $lastNumber = $redis->get($ticketNumberKey);
            ...
            if ($deactivatedTicket) {
                ...
                $redis->set("tickets:counter:$todaysDate", $redisTicketNumber);

                $current_ticket=$redis->get('tickets:counter:$todaysDate');

                \Log::info("Next Ticket In line without skipped Tickets Be $current_ticket");
            } 
            else if (count($msnTicketsArray)>0)
            {
                ...
                \Log::info("Next Ticket In line skipped Tickets Be $current_ticket");
            } 
            else if (count($msnTicketsArray) == 0 && !$duplicateCheck )
            {
                if($redisTicketNumber == 0)
                {
                    $ticketNumber=1;
                ...
                \Log::info("Ticket number is $ticketNumber");

                $redis->set("tickets:counter:$todaysDate", $ticketNumber);
    
                $current_ticket=$redis->get('tickets:counter:$todaysDate');

                \Log::info("Next Ticket In line without skipped Tickets Be $redisTicketNumber");
            }

            // if (count($msnTicketsArray) == 0 && !$duplicateCheck )
            // {
               

            //     // if (!$redis->set("tickets:counter:$todaysDate", $ticket)) {
            ...
            // else if (count($msnTicketsArray) > 0) {
            //     try {
            //         $finalTicketInArray = last($msnTicketsArray); // More Laravel-idiomatic than array_slice
                    
            //         // Use Laravel's Redis facade instead of raw connection
            //         \Illuminate\Support\Facades\Redis::set("tickets:counter:{$todaysDate}", $finalTicketInArray);
                    
            //         // Verify the value was actually set
            //         $current_ticket = \Illuminate\Support\Facades\Redis::get("tickets:counter:{$todaysDate}");
                    
            //         if ($current_ticket === null) {
            //             \Log::error("Failed to retrieve ticket count from Redis for date: {$todaysDate}");
            //             return;
            //         }
                    
            //         \Log::info("Current Ticket {$current_ticket}");
            //         \Log::info("Current Ticket {$current_ticket}. Next Ticket In line with skipped Tickets will Be {$finalTicketInArray}");
            ...
            //     \Log::info("Next Ticket In line with cancelled Tickets will Be $redisTicketNumber");

            //     if (!$redis->set("tickets:counter:$todaysDate", $redisTicketNumber)) {
            //         Log::error('Failed to set Redis key');
            //     }
            // }
        ...
        if($request->submission_type !== null && $request->submission_type == 1)
        {
            ...
            $response_msg = 'Single ticket: Time' . $seconds . ' seconds and microseconds ' . $microseconds.', ticket:'.$ticketNumber.', Last Ticket:'.$lastNumber.', Redis Ticket Number:'.$redisTicketNumber;

            \Log::info($response_msg);

            \Log::info('Skipped tickets are '.json_encode($msnTicketsArray));
