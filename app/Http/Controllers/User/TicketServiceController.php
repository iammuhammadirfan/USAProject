...
class TicketServiceController extends Controller
{
    ...
    // Volunteer Servises 
    public function verify_volunteer_case_number(Request $request)
    {
        // $userIdsArray=json_decode($request->userIdsObject);

        // check if case number exists in the table
        $case_number_data = User::where('case_number', $request->caseNumber)
            ->select('id', 'first_name', 'last_name', 'proxy', 'case_number', 'volunteer_id')
            ->first();
        ...
        if ($case_number_data) {
            // check if user has a ticket already for todays date
            $case_number_ticket = DB::table('volunteer_group_homes_users')
                ->where(['id' => $case_number_data->volunteer_id, 'is_active' => 1])
                ->whereDate('volunteer_group_homes_users.created_at', $parsedDate)
                ->first();
            if ($case_number_ticket) {
                $message = "The Volunteer has already been assigned .";
