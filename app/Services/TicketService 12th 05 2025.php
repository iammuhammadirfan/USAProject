...
class TicketService
{
    ...
    // Add this Lua script as a constant
    ...
    protected function handleSingleTicket($request, $redis, $todaysDate,$lockKey)
    {
        ...
        try {
            ...
            else if (count($msnTicketsArray) == 0)
            {
                ...
                \Log::info($selectedTicketNumber.' is the selected Ticket Number and the current ticket number in redis is '.$existingNumber);
                ...
                if ($selectedTicketNumber == 0) 
                {
                    ...
                    if($checkSelectedTicketCount>0)
                    {
                        $ticketNumber = max($todaysTickets) + 1;;
                ...
                } elseif ($selectedTicketNumber !== $existingNumber && $selectedTicketNumber !== 0) 
                {
                    ...
                    \Log::info($selectedTicketNumber.' is the selected Ticket Number and
                     it not equal to the current ticket number in redis '.$existingNumber);

                    if (in_array($selectedTicketNumber, $todaysTickets)) 
                    {
                        $ticketNumber = max($todaysTickets) + 1;;

                        \Log::info($selectedTicketNumber.' ticket exists in table db , 
                        next ticket will be '.$ticketNumber.' where the selected Ticket Number and
                     it not equal to the current ticket number in redis '.$existingNumber);

                    } else {
                        $ticketNumber = $selectedTicketNumber;

                        \Log::info($selectedTicketNumber.' ticket doesnt exist in table db , next ticket will be '.$ticketNumber.' where the selected Ticket Number and
                     it not equal to the current ticket number in redis '.$existingNumber);
                    }
                }
                ...
                \Log::info($ticketNumber.' for user '.$request->userId.'.Existing ticket was '.$existingNumber);
            } 
